...
void arrayFile(vector<double> &array, char fileName[]){
    ifstream openFile(fileName);
    int count = 0;
    char chr;    

    while(openFile.get(chr)){
        if(chr == '\n'){
            break;
        } count++; }
...
void matrixPrint(const vector<vector<double>> matrix){
      for(size_t i = 0; i < matrix.size(); i++){
        ...
        cout << format("{:10.3}", matrix[i][j]) << "\t"; // выводим пробел между элементами матрицы
    } cout << endl; }
}
